"""
Implementation adapted from https://github.com/lucidrains/vector-quantize-pytorch/blob/master/vector_quantize_pytorch/finite_scalar_quantization.py
"""
...
class FSQ(Module):
    def __init__(
        ...
        self.num_codewords = _levels.prod().item()

        keep_num_codebooks_dim = default(keep_num_codebooks_dim, num_codebooks > 1)
        assert not (num_codebooks > 1 and not keep_num_codebooks_dim)
        self.keep_num_codebooks_dim = keep_num_codebooks_dim

        self.dim = default(embedding_dim, len(_levels) * num_codebooks)
    ...
    def forward(self, z):
        ...
        if not is_img_or_video:
            out = out.squeeze(-1)
            indices = indices.squeeze(-1) if exists(indices) else None
    ...
    def quantize(self, z):
        ...
        zq = self.quantize2zq(z.unsqueeze(-2)) # (b, h*w, c=1, d)
        code = self.codes_to_indices(zq) # (b, h*w, c=1)
        return code.squeeze(-1)
    
    @torch.no_grad()
    def dequantize(self, code):

        zq = self.indices_to_codes(code) # (b, h*w, c=1, d)
        return zq