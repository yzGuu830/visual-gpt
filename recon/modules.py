...
# diffusion modules (copied from VQ-GAN official repo)
...
if __name__ == "__main__":
    ...
    enc = VQGANEncoder(ch=128, out_ch=3, ch_mult=[1,1,2,2,4], num_res_blocks=2, 
                  attn_resolutions=[16], dropout=0.0, in_channels=3, resolution=256, z_channels=256, double_z=False)
    
    dec = VQGANDecoder(ch=128, out_ch=3, ch_mult=[1,1,2,2,4], num_res_blocks=2, attn_resolutions=[16],
                  dropout=0.0, in_channels=3, resolution=256, z_channels=256)
    
    print('encoder num {}M params'.format(sum(p.numel() for p in enc.parameters())/1e6))
    print('decoder num {}M params'.format(sum(p.numel() for p in dec.parameters())/1e6))
