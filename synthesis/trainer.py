...
class Trainer:
    ...
    def train(self):
        ...
        json.dump(namespace2dict(self.conf.transformer), open(os.path.join(self.arg.save_path, 'config.json'), 'w'), indent=4)
        ...
        for x, y in dl: 
            ...
            if pbar.n in self.conf.exp.checkpoint_steps:
                save_checkpoint(self.model.lm, self.arg.save_path, f'model-ckpt-step-{pbar.n}.pth')
